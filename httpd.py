...
def get_file_path_and_ext(root_dir, url):
...
def generate_response(request, root_dir):
    ...
    file_path, ext = get_file_path_and_ext(root_dir, url)
...
def run(server_socket, root_dir):
    event_loop(server_socket, root_dir)
...
if __name__ == '__main__':
    ...
    try:
        [Thread(target=run, args=(server_socket, opts.root_dir)).run() for _ in range(opts.workers)]  # type: ignore
